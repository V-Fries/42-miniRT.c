...
static void	draw_bvh_node(t_engine *engine, t_objects_bvh_node *node, t_vector3f color);
static void	draw_bvh_tree(t_engine *engine, t_objects_bvh_node *root_node, int color_level);
...
//	t_hit bvh_hit = objects_bvh_calculate_ray_intersection(&engine->camera.rays[40000], engine->scene.bvh_tree);

//	printf("HIT:%d\n", bvh_hit.hit);
//	printf("Distance:%f\n", bvh_hit.distance);
//	if (bvh_hit.hit)
//		printf("Type:%u\n", engine->scene.objects.data[bvh_hit.index_obj].type);
//	draw_bvh_tree(engine, engine->scene.bvh_tree, 0);
	(void) draw_bvh_tree;
}
...
static void	draw_bvh_tree(t_engine *engine, t_objects_bvh_node *root_node, int color_level)
{
	...
	t_vector3f	colors[6] = {
			...
			(t_vector3f){0.f, 255.f, 255.f},
			(t_vector3f){255.f, 255.f, 0.f},
			(t_vector3f){0.f, 255.f, 255.f}
	};
