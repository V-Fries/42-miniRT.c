			...
			t_engine *engine, t_click_data click_data)
{
	...
	if (click_data.button != BUTTON_LEFT)
		return (checkered_pattern_size_input_box_y_on_click_text(self, engine,
				click_data));
	object = engine->gui.selected_object.object;
	if (object == NULL)
		return ;
	object->material.checkered_pattern_size.y++;
			...
			t_engine *engine, t_click_data click_data)
{
	...
	if (click_data.button != BUTTON_LEFT)
		return (checkered_pattern_size_input_box_y_on_click_text(self, engine,
				click_data));
	object = engine->gui.selected_object.object;
	if (object == NULL || object->material.checkered_pattern_size.y
		<= FLT_EPSILON)
		return ;
	object->material.checkered_pattern_size.y--;
	engine->scene_changed = true;
			...
			t_engine *engine, t_click_data click_data)
{
	if (click_data.button == SCROLL_UP)
	{
		...
		return (checkered_pattern_size_input_box_y_on_click_plus(self, engine,
				click_data));
	}
	if (click_data.button == SCROLL_DOWN)
	{
		click_data.button = BUTTON_LEFT;
		return (checkered_pattern_size_input_box_y_on_click_minus(self, engine,
				click_data));
	}
}
